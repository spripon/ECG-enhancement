...
def correct_ecg_orientation(image):
    reader = easyocr.Reader(['en'], gpu=False)
    angles = [0, 90, -90, 180]
    keywords = ['D1', 'D2', 'D3', 'aVR', 'aVL', 'aVF', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6', '25 mm/s', '10 mm/mV']
    for angle in angles:
        rotated = rotate_image(image, angle)
        results = reader.readtext(rotated, detail=0)
        detected = [text for text in results if text in keywords]
        if len(detected) >= 3:
            return rotated
    return image
...
if uploaded_file is not None:
    ...
    original = image.copy()
    image = correct_ecg_orientation(image)
